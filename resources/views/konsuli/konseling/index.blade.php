...
@endif
        ...
        {{-- REJECTED HISTORY (Ditolak) --}}
        @if($sessions->where('status', 'rejected')->count() > 0)
        <div id="rejected-section" class="mb-16 max-w-5xl mx-auto animate-fade-in">
            <div class="flex flex-col items-center mb-12">
                <div class="bg-rose-500/10 p-4 rounded-[1.5rem] text-rose-400 border border-rose-500/20 shadow-sm mb-4 backdrop-blur-sm">
                    <i class="fas fa-times-circle text-2xl"></i>
                </div>
                <h2 class="text-3xl font-black text-white tracking-tight text-center">Riwayat Penolakan</h2>
                <div class="w-20 h-1 bg-rose-500/30 rounded-full mt-4"></div>
            </div>
            <div class="bg-[#1e293b] border border-slate-700/50 rounded-2xl shadow-xl overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-700/50">
                        <thead class="bg-slate-800/50">
                            <tr>
                                <th class="px-6 py-5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Konselor</th>
                                <th class="px-6 py-5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Waktu Pengajuan</th>
                                <th class="px-6 py-5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Status</th>
                                <th class="px-6 py-5 text-right text-[11px] font-bold text-slate-400 uppercase tracking-wider">Opsi</th>
                            </tr>
                        </thead>
                        <tbody id="rejected-list" class="divide-y divide-slate-700/50">
                            @foreach($sessions->where('status', 'rejected') as $session)
                            <tr id="konseli-session-{{ $session->id }}" class="hover:bg-slate-800/50 transition-colors group">
                                <td class="px-6 py-5">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 rounded-full bg-slate-800 mr-4 overflow-hidden border border-slate-700">
                                            @if($session->konselor->foto)
                                        ...
                                        <div>
                                            <div class="text-sm font-semibold text-white">{{ $session->konselor->nama }}</div>
                                            <div class="text-[11px] text-slate-400 mt-0.5">ID: {{ $session->konselor->npm_nip }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="text-sm text-slate-300">{{ $session->created_at?->translatedFormat('d M Y') ?? 'Baru saja' }}</div>
                                    <div class="text-xs text-slate-500 mt-1">{{ $session->created_at?->format('H:i') ?? '--:--' }} WIB</div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-right">
                                    <a href="{{ route('konsuli.konselor.index') }}"
                                       class="inline-flex items-center px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 hover:border-slate-600 rounded-lg transition-colors text-sm font-medium shadow-sm">
                                        Cari Lain
                                    </a>
                                </td>
                            </tr>
            ...
            <div class="bg-[#1e293b] border border-slate-700/50 rounded-2xl shadow-xl overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-700/50">
                        <thead class="bg-slate-800/50">
                            <tr>
                                <th class="px-6 py-5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Konselor Sahabat</th>
                                <th class="px-6 py-5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Kapan?</th>
                                <th class="px-6 py-5 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Durasi Cerita</th>
                                <th class="px-6 py-5 text-right text-[11px] font-bold text-slate-400 uppercase tracking-wider">Opsi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-700/50">
                            ...
                            <tr class="hover:bg-slate-800/50 transition-colors group">
                                ...
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="text-sm text-slate-300">{{ $session->started_at?->translatedFormat('d M Y') }}</div>
                                    <div class="text-xs text-slate-500 mt-1">Pukul {{ $session->started_at?->format('H:i') }} WIB</div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
...
/* ── BARU: Tabel Ditolak ──────────────────────────────── */
function buildRejectedRow(s) {
    let day = 'Baru saja';
    let time = '--:--';
    if (s.created_at) {
        const dateObj = new Date(s.created_at);
        day = dateObj.toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' });
        time = dateObj.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }).replace('.', ':');
    }
    
    return `
    <tr id="konseli-session-${s.id}" class="hover:bg-slate-800/50 transition-colors group animate-fade-in-down">
        <td class="px-6 py-5">
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-full bg-slate-800 mr-4 overflow-hidden border border-slate-700">
                    ${avatarHtml(s.konselor?.foto, 'bg-slate-800', 'fas fa-user')}
                </div>
                <div>
                    <div class="text-sm font-semibold text-white">${escHtml(s.konselor?.nama)}</div>
                    <div class="text-[11px] text-slate-400 mt-0.5">ID: ${escHtml(s.konselor?.npm_nip)}</div>
                </div>
            </div>
        </td>
        <td class="px-6 py-5 whitespace-nowrap">
            <div class="text-sm text-slate-300">${day}</div>
            <div class="text-xs text-slate-500 mt-1">${time} WIB</div>
        </td>
        <td class="px-6 py-5 whitespace-nowrap">
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20">
                Ditolak
...
<div id="rejected-section" class="mb-16 max-w-5xl mx-auto animate-fade-in-down">
            ...
            .listen('.session.updated', (s) => {
                ...
                // ── REJECTED: tampilkan card ditolak ───────────
                    ...
                    setTimeout(() => {
                        ...
                        // Pastikan section "Ditolak" sudah ada, baru tambahkan tabel
                        ensureSection('rejected-section', REJECTED_SECTION, 'afterend');
                        const rl = document.getElementById('rejected-list');
                        if (rl && !document.getElementById(`konseli-session-${s.id}`)) {
                            rl.insertAdjacentHTML('afterbegin', buildRejectedRow(s));
                        }
