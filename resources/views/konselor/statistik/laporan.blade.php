...
{{-- ═══════════════════════════════════════════
     ROW 5 — Kategori Unit + Aktivitas Konselor
...
<div class="chart-row chart-row-3 section-break-before" style="margin-bottom:2rem;">
    ...
    {{-- Aktivitas Konselor / Penanganan --}}
    <div class="card anim-card" style="padding:1.375rem;">
        <div class="sec-header">
            <i class="fas fa-user-shield"></i> Aktivitas Penanganan Konselor
            <span class="sec-badge">{{ $aktivitasKonselor->count() }} konselor</span>
        </div>
        <div style="overflow-x:auto;">
            <table class="konselor-table">
                ...
                <tbody>
                    @forelse($aktivitasKonselor as $i => $a)
                    <tr>
                        <td>
...
/* ── Cetak PDF ── */
function cetakPDF() {
    // Paksa semua canvas ter-render dulu, baru konversi
...
function gantiPeriode(periode) {
    ...
    window.location.href = '{{ route("konselor.statistik.laporan") }}?periode=' + periode;
}
