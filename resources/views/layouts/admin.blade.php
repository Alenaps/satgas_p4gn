    ...
    <aside class="w-full md:w-64 bg-gradient-to-b from-green-600 to-green-700 text-white flex flex-col shadow-xl">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.laporan.index') }}" 
               class="flex items-center gap-3 px-3 py-2.5 hover:bg-green-500 rounded-lg transition-colors {{ request()->routeIs('laporan.*') ? 'bg-green-500' : '' }}">
                <i class="fas fa-file-alt w-5"></i>
                <span>Data Laporan</span>
            </a>

            <a href="{{ route('admin.kelola_pengguna.index') }}" 
               class="flex items-center gap-3 px-3 py-2.5 hover:bg-green-500 rounded-lg transition-colors {{ request()->routeIs('konseling.*') ? 'bg-green-500' : '' }}">
