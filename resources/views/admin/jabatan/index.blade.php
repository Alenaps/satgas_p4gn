...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Master Data Jabatan</h2>
            <p class="text-sm text-gray-500 mt-0.5">Kelola data jabatan konselor</p>
        </div>
    ...
    {{-- Search Bar --}}
    <form method="GET" action="{{ route('admin.jabatan.index') }}" class="flex gap-3 mb-5">
    ...
    @if(request('search'))
        <p class="text-sm text-gray-500 mb-3">
            Menampilkan <span class="font-semibold text-gray-700">{{ $jabatans->total() }}</span> hasil
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="bg-gradient-to-r from-green-600 to-green-700 text-white">
                    ...
                    <th class="px-6 py-3.5 font-semibold">Nama Jabatan</th>
            ...
            <tbody>
                @forelse($jabatans as $jabatan)
                    <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }} hover:bg-green-50 transition-colors border-b border-gray-100 last:border-0">
                        ...
                        <td class="px-6 py-4 font-semibold text-gray-800">{{ $jabatan->nama_jabatan }}</td>
                    ...
                    <tr>
                        <td colspan="3" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-2 text-gray-400">
                                ...
                                @if(request('search'))
                                    <a href="{{ route('admin.jabatan.index') }}" class="text-xs text-green-600 hover:underline mt-1">Tampilkan semua jabatan</a>
                                @endif
                            </div>
                        </td>
                    </tr>
        ...
        @if($jabatans->total() > 0)
            <div class="px-6 py-3.5 border-t border-gray-100 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-3">
                <p class="text-xs text-gray-500">
                    Menampilkan <span class="font-semibold text-gray-700">{{ $jabatans->firstItem() }}–{{ $jabatans->lastItem() }}</span>
